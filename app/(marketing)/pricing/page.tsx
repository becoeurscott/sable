    ...
    <div style={{ animation: "sbFade .4s ease both" }}>
      <div style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1160, margin: "0 auto", padding: "66px 28px 30px", textAlign: "center", position: "relative" }}>
          <h1 className="sb-h2" style={{ fontFamily: DISPLAY, fontSize: 48, lineHeight: 1.05, letterSpacing: "-.03em", fontWeight: 700, margin: "0 0 14px" }}>Simple pricing that scales with you</h1>
          ...
          <div style={{ display: "inline-flex", background: "#F0F3F9", border: "1px solid #E4E9F2", borderRadius: 12, padding: 4, gap: 4 }}>
            <button
              ...
              style={{ border: "none", cursor: "pointer", fontWeight: 600, fontSize: 14, padding: "9px 20px", borderRadius: 9, background: !annual ? "#fff" : "transparent", color: !annual ? "#0B1220" : "#71798A", transition: "background .25s ease, color .25s ease" }}
            >
              Monthly
            </button>
            <button
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "14px 28px 40px" }}>
        <div className="r-5" style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 14, alignItems: "stretch" }}>
          {plans.map((p, i) => (
            <Reveal key={p.name} delay={i * 70} style={{ display: "flex" }}>
            <div className="sb-card" style={{ background: p.cardBg, border: p.border, borderRadius: 18, padding: "24px 20px", display: "flex", flexDirection: "column", position: "relative", boxShadow: p.shadow, width: "100%" }}>
              {p.popular && (
        ...
        <div className="r-3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
          {addons.map((a, i) => (
            <Reveal key={a.name} delay={(i % 3) * 80}>
              <div className="sb-card" style={{ border: "1px solid #EBEFF6", background: "#F7F9FC", borderRadius: 14, padding: "18px 20px", height: "100%" }}>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "50px 28px 80px" }}>
        ...
        {faqs.map((q, i) => (
          <Reveal key={q.q} delay={i * 60}>
            <div style={{ borderBottom: "1px solid #EDF0F6", padding: "20px 0" }}>
